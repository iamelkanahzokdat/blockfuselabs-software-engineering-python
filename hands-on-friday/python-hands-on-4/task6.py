...
Lami = cart
Chinedu = cart
cart["phones"].update({"Tobi":("iphone",750000)})
cart["laptops"].update({"Lami":("Dell XPS",1200000)})
cart["phones"].update({"Chinedu":("Earbuds",50000)})
cart["phones"].pop("Tobi")
cart["accessories"].update({"Lami":("Mouse",35000)})
snapshot = cart.copy()
print(snapshot)
print(hex(id(Tobi)))
print(hex(id(Lami)))
print(hex(id(Chinedu)))
print(hex(id(snapshot)))
cart.clear()
print(cart)
